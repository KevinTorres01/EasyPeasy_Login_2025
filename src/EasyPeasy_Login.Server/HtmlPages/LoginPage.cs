...
{
    /// <summary>
    /// Página de Login convertida de Blazor a HTML puro para servidor HTTP con sockets
    /// Incluye TODAS las animaciones: eye tracking, mouth opening, pupil scaling
    /// </summary>
    public static class LoginPage
    {
        // SVG Icons
        private const string CROWN_SVG = "<svg xmlns='http://www.w3.org/2000/svg' width='48' height='48' fill='#000000' viewBox='0 0 256 256'><path d='M239.75,90.81c0,.11,0,.21-.07.32L217,195a16,16,0,0,1-15.72,13H54.71A16,16,0,0,1,39,195L16.32,91.13c0-.11-.05-.21-.07-.32A16,16,0,0,1,44,77.39l33.67,36.29,35.8-80.29a1,1,0,0,0,0-.1,16,16,0,0,1,29.06,0,1,1,0,0,0,0,.10l35.8,80.29L212,77.39a16,16,0,0,1,27.71,13.42Z'></path></svg>";
        
        private const string EYE_SVG = "<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='#000000' viewBox='0 0 256 256'><path d='M247.31,124.76c-.35-.79-8.82-19.58-27.65-38.41C194.57,61.26,162.88,48,128,48S61.43,61.26,36.34,86.35C17.51,105.18,9,124,8.69,124.76a8,8,0,0,0,0,6.5c.35.79,8.82,19.57,27.65,38.4C61.43,194.74,93.12,208,128,208s66.57-13.26,91.66-38.34c18.83-18.83,27.3-37.61,27.65-38.4A8,8,0,0,0,247.31,124.76ZM128,192c-30.78,0-57.67-11.19-79.93-33.25A133.47,133.47,0,0,1,25,128,133.33,133.33,0,0,1,48.07,97.25C70.33,75.19,97.22,64,128,64s57.67,11.19,79.93,33.25A133.46,133.46,0,0,1,231.05,128C223.84,141.46,192.43,192,128,192Zm0-112a48,48,0,1,0,48,48A48.05,48.05,0,0,0,128,80Zm0,80a32,32,0,1,1,32-32A32,32,0,0,1,128,160Z'></path></svg>";
        
        private const string EYE_SLASH_SVG = "<svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='#000000' viewBox='0 0 256 256'><path d='M53.92,34.62A8,8,0,1,0,42.08,45.38L61.32,66.55C25,88.84,9.38,123.2,8.69,124.76a8,8,0,0,0,0,6.5c.35.79,8.82,19.57,27.65,38.4C61.43,194.74,93.12,208,128,208a127.11,127.11,0,0,0,52.07-10.83l22,24.21a8,8,0,1,0,11.84-10.76Zm47.33,75.84,41.67,45.85a32,32,0,0,1-41.67-45.85ZM128,192c-30.78,0-57.67-11.19-79.93-33.25A133.16,133.16,0,0,1,25,128c4.69-8.79,19.66-33.39,47.35-49.38l18,19.75a48,48,0,0,0,63.66,70l14.73,16.2A112,112,0,0,1,128,192Zm6-95.43a8,8,0,0,1,3-15.72,48.16,48.16,0,0,1,38.77,42.64,8,8,0,0,1-7.22,8.71,6.39,6.39,0,0,1-.75,0,8,8,0,0,1-8-7.26A32.09,32.09,0,0,0,134,96.57Zm113.28,34.69c-.42.94-10.55,23.37-33.36,43.8a8,8,0,1,1-10.67-11.92A132.77,132.77,0,0,0,231.05,128a133.15,133.15,0,0,0-23.12-30.77C185.67,75.19,158.78,64,128,64a118.37,118.37,0,0,0-19.36,1.57A8,8,0,1,1,106,49.79,134,134,0,0,1,128,48c34.88,0,66.57,13.26,91.66,38.35,18.83,18.83,27.3,37.62,27.65,38.41A8,8,0,0,1,247.31,131.26Z'></path></svg>";

        /// <summary>
        /// Genera la página de login limpia (sin errores)
        /// </summary>
        /// <param name="clientIp">IP del cliente detectada</param>
        /// <param name="clientMac">MAC del cliente detectada</param>
        /// <returns>HTML completo de la página</returns>
        public static string GenerateCleanLoginPage(string clientIp = "", string clientMac = "")
        {
            return GenerateLoginPageInternal("", "", clientIp, clientMac, false);
        }

        /// <summary>
        /// Genera la página de login con un mensaje de error
        /// </summary>
        /// <param name="errorMessage">Mensaje de error a mostrar</param>
        /// <param name="username">Username para mantener en el campo</param>
        /// <param name="clientIp">IP del cliente detectada</param>
        /// <param name="clientMac">MAC del cliente detectada</param>
        /// <returns>HTML completo de la página con el error</returns>
        public static string GenerateLoginPageWithError(string errorMessage, string username = "", string clientIp = "", string clientMac = "")
        {
            return GenerateLoginPageInternal(errorMessage, username, clientIp, clientMac, false);
        }

        private static string GenerateLoginPageInternal(string errorMessage, string username, string clientIp, string clientMac, bool acceptTerms)
        {
            ...
            html.AppendLine("    <title>Welcome back!</title>");
            html.AppendLine("    <style>");
            html.AppendLine("        /* Reset global */");
            html.AppendLine("        * { margin: 0; padding: 0; box-sizing: border-box; }");
            html.AppendLine("        html, body { height: 100%; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; }");
            html.AppendLine("        body { margin: 0; padding: 0; }");
            html.AppendLine("");
            html.AppendLine("        /* login page */");
            html.AppendLine("        .login-page { min-height: 100vh; background: #6b7280; display: flex; align-items: center; justify-content: center; padding: 20px; }");
            html.AppendLine("        .login-container { display: flex; width: 100%; max-width: 1100px; min-height: 600px; background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); }");
            html.AppendLine("");
            html.AppendLine("        /* left panel */");
            html.AppendLine("        .left-panel { flex: 1.5; background: rgb(231,231,231); display: flex; align-items: center; justify-content: center; padding: 10px; }");
            html.AppendLine("        .avatar-area { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; }");
            html.AppendLine("        .avatars-svg { width: 100%; height: auto; }");
            html.AppendLine("");
            html.AppendLine("        /* Animaciones de avatares */");
            html.AppendLine("        .pupil { transition: transform 0.08s ease-out; transform-origin: center; transform-box: fill-box; }");
            html.AppendLine("        .mouth-part { transition: transform 0.2s ease-out; transform-origin: center; transform-box: fill-box; }");
            html.AppendLine("");
            html.AppendLine("        /* right panel */");
            html.AppendLine("        .right-panel { flex: 1; display: flex; align-items: center; justify-content: center; padding: 40px; background: white; }");
            html.AppendLine("        .login-card { width: 100%; max-width: 400px; }");
            html.AppendLine("");
            html.AppendLine("        /* crown */");
            html.AppendLine("        .crown-icon { text-align: center; margin-bottom: 24px; }");
            html.AppendLine("        .crown-icon svg { width: 48px; height: 48px; }");
            html.AppendLine("");
            html.AppendLine("        /* titles */");
            html.AppendLine("        .login-title { font-size: 32px; font-weight: 700; color: #000; text-align: center; margin-bottom: 8px; }");
            html.AppendLine("        .login-subtitle { font-size: 14px; color: #6b7280; text-align: center; margin-bottom: 32px; }");
            html.AppendLine("");
            html.AppendLine("        /* form */");
            html.AppendLine("        .login-form { width: 100%; }");
            html.AppendLine("        .form-group { margin-bottom: 24px; }");
            html.AppendLine("        .form-label { display: block; font-size: 14px; font-weight: 600; color: #000; margin-bottom: 8px; }");
            html.AppendLine("        .form-input { width: 100%; padding: 12px 0; font-size: 16px; color: #000; background: transparent; border: none; border-bottom: 2px solid #d1d5db; outline: none; transition: border-color 0.2s; }");
            html.AppendLine("        .form-input:focus { border-bottom-color: #000; }");
            html.AppendLine("");
            html.AppendLine("        /* Password wrapper */");
            html.AppendLine("        .password-wrapper { position: relative; display: flex; align-items: center; }");
            html.AppendLine("        .password-toggle { position: absolute; right: 0; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; padding: 8px; opacity: 0.6; transition: opacity 0.2s; }");
            html.AppendLine("        .password-toggle:hover { opacity: 1; }");
            html.AppendLine("        .password-toggle svg { width: 20px; height: 20px; display: block; }");
            html.AppendLine("");
            html.AppendLine("        /* Checkbox */");
            html.AppendLine("        .form-check { margin: 20px 0; }");
            html.AppendLine("        .checkbox-container { display: flex; align-items: center; cursor: pointer; position: relative; padding-left: 28px; font-size: 14px; color: #6b7280; user-select: none; }");
            html.AppendLine("        .checkbox-container input[type='checkbox'] { position: absolute; opacity: 0; cursor: pointer; height: 0; width: 0; }");
            html.AppendLine("        .checkmark { position: absolute; left: 0; top: 50%; transform: translateY(-50%); height: 18px; width: 18px; border: 2px solid #d1d5db; border-radius: 4px; background: white; transition: all 0.2s; }");
            html.AppendLine("        .checkbox-container:hover .checkmark { border-color: #000; }");
            ...
            html.AppendLine("        @keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-10px); } 75% { transform: translateX(10px); } }");
            html.AppendLine("");
            html.AppendLine("        /* button */");
            html.AppendLine("        .btn-login { width: 100%; padding: 14px 24px; background: #000; color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; }");
            html.AppendLine("        .btn-login:hover:not(:disabled) { background: #1a1a1a; transform: translateY(-2px); box-shadow: 0 8px 16px rgba(0, 0, 0, 0.2); }");
            html.AppendLine("        .btn-login:disabled { opacity: 0.6; cursor: not-allowed; }");
            html.AppendLine("");
            html.AppendLine("        /* spinner */");
            html.AppendLine("        .spinner { width: 16px; height: 16px; border: 2px solid rgba(255, 255, 255, 0.3); border-top-color: white; border-radius: 50%; animation: spin 0.6s linear infinite; }");
            html.AppendLine("        @keyframes spin { to { transform: rotate(360deg); } }");
            html.AppendLine("");
            html.AppendLine("        /* Debug info */");
            html.AppendLine("        .debug-info { margin-top: 24px; padding: 16px; background: #f9fafb; border-radius: 8px; text-align: center; }");
            html.AppendLine("        .debug-info small { display: block; color: #6b7280; font-size: 12px; margin: 4px 0; }");
            html.AppendLine("");
            html.AppendLine("        /* Devil Avatar */");
            html.AppendLine("        .devil-avatar { position: fixed; bottom: -250px; right: -150px; width: 200px; height: 267px; transform: rotate(-25deg); transition: all 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55); z-index: 1000; pointer-events: none; opacity: 0; }");
            html.AppendLine("        .devil-avatar.show { bottom: -50px; right: -20px; opacity: 1; }");
            html.AppendLine("        .devil-avatar svg { width: 100%; height: 100%; filter: drop-shadow(2px 2px 4px rgba(0,0,0,0.3)); }");
            html.AppendLine("        .devil-avatar .devil-pupil { transition: transform 0.08s ease-out; transform-origin: center; transform-box: fill-box; }");
            html.AppendLine("        .devil-avatar.animating .eyebrow { animation: eyebrowBounce 0.3s ease-in-out 2; }");
            html.AppendLine("        @keyframes eyebrowBounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }");
            html.AppendLine("");
            html.AppendLine("        /* Responsive */");
            html.AppendLine("        @media (max-width: 768px) {");
            html.AppendLine("            .login-container { flex-direction: column; }");
            html.AppendLine("            .left-panel { min-height: 200px; }");
            html.AppendLine("            .login-title { font-size: 28px; }");
            html.AppendLine("        }");
            html.AppendLine("    </style>");
            html.AppendLine("</head>");
            html.AppendLine("<body>");
            html.AppendLine("    <div class='login-page'>");
            html.AppendLine("        <div class='login-container'>");
            html.AppendLine("            <!-- Left panel -->");
            html.AppendLine("            <div class='left-panel'>");
            html.AppendLine("                <div class='avatar-area'>");
            
            // Insertar el SVG completo de los avatares
            html.AppendLine(GetAvatarsSVG());
            
            html.AppendLine("                </div>");
            html.AppendLine("            </div>");
            html.AppendLine("");
            html.AppendLine("            <!-- Right panel -->");
            html.AppendLine("            <div class='right-panel'>");
            html.AppendLine("                <div class='login-card'>");
            html.AppendLine("                    <!-- Crown -->");
            html.AppendLine($"                    <div class='crown-icon'>{CROWN_SVG}</div>");
            html.AppendLine("");
            html.AppendLine("                    <!-- Titles -->");
            html.AppendLine("                    <h1 class='login-title'>Welcome back!</h1>");
            ...
            html.AppendLine($"                            <input type='text' name='username' class='form-input' value='{EscapeHtml(username)}' required />");
            html.AppendLine("                        </div>");
            html.AppendLine("");
            html.AppendLine("                        <!-- Password -->");
            html.AppendLine("                        <div class='form-group'>");
            html.AppendLine("                            <label class='form-label'>Password</label>");
            html.AppendLine("                            <div class='password-wrapper'>");
            html.AppendLine("                                <input type='password' name='password' class='form-input' id='password-input' required />");
            html.AppendLine("                                <button type='button' class='password-toggle' id='password-toggle'>");
            html.AppendLine($"                                    <span id='eye-icon'>{EYE_SLASH_SVG}</span>");
            html.AppendLine("                                </button>");
            html.AppendLine("                            </div>");
            html.AppendLine("                        </div>");
            html.AppendLine("");
            ...
            html.AppendLine("");
            html.AppendLine("                        <!-- Login button -->");
            html.AppendLine("                        <button type='submit' class='btn-login' id='login-button'>");
            html.AppendLine("                            <span>Log in</span>");
            html.AppendLine("                        </button>");
            html.AppendLine("                    </form>");
            html.AppendLine("");
            html.AppendLine("                    <!-- Debug -->");
            html.AppendLine("                    <div class='debug-info'>");
            html.AppendLine("                        <small>Debug: admin / admin05</small>");
            html.AppendLine($"                        <small>IP: {EscapeHtml(clientIp)}</small>");
            html.AppendLine($"                        <small>MAC: {EscapeHtml(clientMac)}</small>");
            html.AppendLine("                    </div>");
            html.AppendLine("                </div>");
            html.AppendLine("            </div>");
            html.AppendLine("        </div>");
            ...
            html.AppendLine("");
            html.AppendLine("    <!-- Devil Avatar -->");
            html.AppendLine(GetDevilAvatarHTML());
            html.AppendLine("");
        ...
        private static string GetAvatarsSVG()
        {
            return @"<svg class='avatars-svg' viewBox='0 0 520 360' xmlns='http://www.w3.org/2000/svg' aria-hidden='true'>
  <defs id='defs2183' />
  ...
  <rect style='fill:#e8e9ed;fill-opacity:1;stroke-width:0.674335' id='rect3490' width='482.37091' height='24.073071' x='15.045849' y='325.81213' rx='0' />
        ...
        private static string GetDevilAvatarHTML()
        {
            return @"<div class='devil-avatar' id='devil-avatar'>
...
";
        ...
        /// <summary>
        /// Parsea los datos POST del formulario de login
        /// </summary>
        /// <param name="postData">Datos en formato: username=admin&password=123&acceptTerms=on</param>
        /// <returns>Objeto LoginFormData con los valores parseados</returns>
        public static LoginFormData? ParseLoginFormData(string postData)
        {
            ...
            foreach (var pair in pairs)
            {
                ...
                switch (key.ToLower())
                {
                    ...
                    case "password":
                        ...
                        break;
                    case "acceptterms":
                        data.AcceptTerms = value.ToLower() == "on" || value.ToLower() == "true";
                        break;
                }
            }
        ...
        /// <summary>
        /// Clase para almacenar los datos del formulario de login
        /// </summary>
